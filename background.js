/* ===== FaultyTerminal desktop background — vanilla-JS port of React Bits FaultyTerminal =====
 * Shader/geometry logic below is the source component's WebGL code, unchanged; only the
 * React wrapper (props/refs/useEffect) was stripped and replaced with a plain init call.
 * Dependency: ogl (plain WebGL helper module, loaded via CDN as an ES module — no React). */
...
// ponytail: fixed CyberOS theme config instead of a full props API — this is a background,
// not a reusable component; add props back if a second usage ever needs different values.
const CONFIG = {
  scale: 1.5,
  gridMul: [2, 1],
  digitSize: 1.2,
...
function initFaultyTerminal(container) {
  ...
  if (CONFIG.mouseReact) container.addEventListener("pointermove", handleMouseMove);

  function update(t) {
    requestAnimationFrame(update);

    if (CONFIG.pageLoadAnimation && loadAnimationStart === 0) loadAnimationStart = t;
...
initFaultyTerminal(document.getElementById("bgTerminal"));
