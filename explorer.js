/* ===== Cyber Explorer — vanilla-JS port of React Bits "InfiniteMenu" =====
 * Geometry/ArcballControl/InfiniteGridMenu below are unchanged WebGL/vanilla-JS
 * logic from the source component; only the React wrapper (state/refs/useEffect)
 * was stripped and replaced with plain DOM wiring at the bottom of this file.
 * Dependency: gl-matrix (plain math module, loaded via CDN script, no React). */
      ...
      this.items.map(
        (item) =>
          new Promise((resolve) => {
            ...
            // ponytail: resolve (not reject) on error — one bad thumbnail must not
            // hang Promise.all forever and leave the whole sphere black.
...
/* ---- vanilla wiring (replaces the React InfiniteMenu component) ---- */
let explorerMenu = null;

function initExplorer() {
  const canvas = document.getElementById("explorerCanvas");
  const titleEl = document.getElementById("explorerCardTitle");
  const descEl = document.getElementById("explorerCardDesc");
  const linkEl = document.getElementById("explorerCardLink");
  const searchEl = document.getElementById("explorerSearch");

  let currentItems = CYBER_RESOURCES;
  ...
  const setMoving = (isMoving) => {
    const cls = isMoving ? "inactive" : "active";
    const clsOpp = isMoving ? "active" : "inactive";
    [titleEl, descEl, linkEl].forEach((el) => { el.classList.add(cls); el.classList.remove(clsOpp); });
  };

  try {
    explorerMenu = new InfiniteGridMenu(canvas, currentItems, setCard, setMoving, (menu) => menu.run());
    window.explorerMenu = explorerMenu;
    setMoving(false); // ponytail: onMovementChange only fires on a state transition,
    // and isMoving starts false (matching the default) — force the overlay visible on load.
  } catch (err) {
    canvas.replaceWith(Object.assign(document.createElement("p"), {
      className: "error-text",
      textContent: "WebGL2 not available in this browser — Cyber Explorer needs it."
    }));
    console.error(err);
    return;
  }

  window.addEventListener("resize", () => explorerMenu && explorerMenu.resize());

  searchEl.addEventListener("input", () => {
    const q = searchEl.value.trim().toLowerCase();
    const filtered = q
      ? CYBER_RESOURCES.filter((r) => r.title.toLowerCase().includes(q) || r.tags.toLowerCase().includes(q))
      : CYBER_RESOURCES;
    if (filtered.length === 0) return; // ponytail: empty result keeps last sphere rather than divide-by-zero
    currentItems = filtered;
    explorerMenu.updateItems(filtered);
  });
}
